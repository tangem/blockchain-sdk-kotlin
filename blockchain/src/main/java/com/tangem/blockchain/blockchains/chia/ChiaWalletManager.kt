...
class ChiaWalletManager(
    ...
    override fun checkUtxoAmountLimit(amount: BigDecimal, fee: BigDecimal): UtxoAmountLimit {
        ...
        val amountLimit = UtxoAmountLimit(
            limit = ChiaTransactionBuilder.MAX_INPUT_COUNT.toBigDecimal(),
            availableToSpend = transactionBuilder.getUnspentsToSpendAmount().toBigDecimal(),
            availableToSend = null,
        )
        return if (changeDecimal < BigDecimal.ZERO) { // unspentsToSpend not enough to cover transaction amount
            amountLimit.copy(availableToSend = amount + changeDecimal)
        } else {
            amountLimit
        }
    }
