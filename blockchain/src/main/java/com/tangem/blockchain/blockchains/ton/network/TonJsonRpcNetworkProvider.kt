...
class TonJsonRpcNetworkProvider(
    ...
    override suspend fun getJettonBalance(jettonWalletAddress: String): Result<BigInteger> {
        return try {
            ...
            if (response.exitCode == RESPONSE_EXIT_CODE) {
    ...
    private companion object {
        const val GET_WALLET_ADDRESS_METHOD = "get_wallet_address"
        const val GET_WALLET_DATA_METHOD = "get_wallet_data"
        const val RESPONSE_EXIT_CODE = -13 // returns when contract where call get does not exist
    }
}