...
internal open class EthereumNetworkService(
    ...
    private val yieldLendingProvider: YieldLendingProvider = DefaultYieldLendingProvider,
    ...
    private suspend fun getTokensBalanceInternal(address: String, tokens: Set<Token>): List<Amount> {
        return coroutineScope {
            val yieldContract = yieldLendingProvider.getYieldContract()

            tokens.map { token ->
                async {
                    if (yieldContract != EthereumUtils.EMPTY_ADDRESS) {
                        val isLent = yieldLendingProvider.isLent(token)

                        if (isLent) {
                            yieldLendingProvider.getLentBalance(token)
                        } else {
