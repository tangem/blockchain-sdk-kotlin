...
internal class KaspaWalletManager(
    ...
    override fun checkUtxoAmountLimit(amount: BigDecimal, fee: BigDecimal): UtxoAmountLimit {
        ...
        val amountLimit = UtxoAmountLimit(
            maxLimit = KaspaTransactionBuilder.MAX_INPUT_COUNT.toBigDecimal(),
            maxAmount = null,
            maxAvailableAmount = transactionBuilder.getUnspentsToSpendAmount(),
        )
        val change = try {
            transactionBuilder.calculateChange(amount, fee, dustValue, unspents)
        } catch (ex: BlockchainError) {
            return amountLimit
        }
        return if (change < BigDecimal.ZERO) { // unspentsToSpend not enough to cover transaction amount
            amountLimit.copy(maxAmount = amount + change - dustValue)
        } else {
            amountLimit
        }
    }
