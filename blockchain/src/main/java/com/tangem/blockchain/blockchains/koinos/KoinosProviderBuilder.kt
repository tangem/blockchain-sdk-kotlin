...
internal class KoinosProviderBuilder(
...
) : NetworkProvidersBuilder<KoinosNetworkProvider>() {

    override fun createProviders(blockchain: Blockchain): List<KoinosNetworkProvider> {
    ...
    override fun createTestnetProviders(blockchain: Blockchain): List<KoinosNetworkProvider> {
        return listOf(
    ...
    private fun createProvider(baseUrl: String, isTestnet: Boolean, apiKey: String? = null): KoinosNetworkProvider {
        return KoinosNetworkProvider(baseUrl = baseUrl, apiKey = apiKey, isTestnet = isTestnet)
    }
}